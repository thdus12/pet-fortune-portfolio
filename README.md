...
### 프롬프트 엔지니어링 — LLM 콘텐츠 품질 관리

12종 × 5개 언어 = **60개 프롬프트**, 한국어 기준 **2,660줄** 규모의 프롬프트 시스템을 설계·운영합니다.

| 구분 | 설명 |
|------|------|
| **역할 분리** | WRITER(분석+글쓰기)와 REVIEWER(말투·리듬 교정)를 분리하여 품질 관리 |
| **행성별 해석 규칙** | 태양/달/수성/금성/화성 각각의 해석 범위를 섹션별로 제한 (예: communication은 수성만, conflict는 수성+화성) |
| **원소별 해석 범위** | 불/공기/흙/물 원소에 따른 허용·금지 표현을 명시하여 hallucination 방지 |
| **컨텍스트 주입** | jobStatus(직업), businessRelation(직장 내 관계), relationshipStatus(연애 상태)를 프롬프트에 전달하여 맥락에 맞는 분석 유도 |
| **점수-내용 일치** | 본문 톤과 점수의 일관성 규칙을 명시하여 "긍정적으로 썼는데 낮은 점수" 같은 모순 방지 |
| **다국어 동기화** | 한국어 마스터 프롬프트를 기준으로 분석 규칙은 동일 유지, 톤/문체만 로컬라이제이션 |

### Claude Code — AI 페어 프로그래밍

프로젝트 전반의 개발 워크플로우에 Claude Code를 적극 활용합니다.

| 활용 영역 | 내용 |
|----------|------|
| **Custom Skills** | `/commit` — 변경사항 분석 후 커밋 규칙(COMMIT_ROLES.md)에 따라 자동 커밋 메시지 생성 및 기능별 분리 커밋 |
| | `/api-update` — 백엔드 OpenAPI 스펙 갱신 → Orval로 TypeScript 클라이언트 자동 재생성 |
| **풀스택 개발** | 프론트엔드(Vue 3) + 백엔드(Spring Boot) + 프롬프트를 하나의 컨텍스트에서 동시 수정 |
| **프롬프트 엔지니어링** | 60개 다국어 프롬프트의 일관성 유지, 새로운 해석 규칙 추가 시 5개 언어 동시 반영 |
| **코드 리뷰 & 개선** | UX 개선 제안 (알럿 → 버튼 비활성화), 입력 검증 로직 리팩토링, CSS 이슈 수정 |
