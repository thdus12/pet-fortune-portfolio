...
**꼬순내 신당**은 천체 위치 계산(NASA JPL Ephemeris)과 LLM을 활용하여, 깊이 있는 별자리 성격 분석 리포트를 생성하는 AI 콘텐츠 플랫폼입니다.
...
### 1. 별자리 성격 분석 리포트
출생 시간/장소 기반 네이탈 차트를 계산하고, 태양/달/상승/수성/금성/화성 + 12하우스 배치를 분석하여 10개 섹션의 성격 리포트를 생성합니다.
...
- 나이대별 프롬프트 분기 (10대 / 20대 / 30대+) 로 톤과 표현 차별화
- 태양/달/상승 가중치(50/30/20%) 적용
- 핵심 어스펙트(orb < 2°) 별도 강조

### 2. 사람 궁합 분석
두 사람의 출생 정보를 기반으로 Synastry(상성) + Composite(합성) 차트를 계산하여 궁합 리포트를 생성합니다.

- **관계 유형**: 연인 / 가족 / 친구 / 비즈니스 파트너
- **양음력 지원**: 한국 음력 달력 변환 내장
- **상태 반영**: 연애 상태(솔로/연애중/기혼), 직업 상태를 반영

### 3. 신년 운세
연간 트랜짓 행성 배치를 기반으로 12개월 총운 + 월별 핵심 키워드를 제공합니다.

- 영역별 점수 산출 (커리어, 재정, 관계, 건강)
- 상반기/하반기 요약
- 핵심 월(Key Month) 하이라이트

### 4. 이번달 종합 해설
해당 월의 트랜짓 데이터를 기반으로 7개 생활 영역(커리어/재정/관계/건강/영성/학업/모험)을 상세 분석합니다.

- 피크 날짜(Peak Date) 안내
- 트랜짓 타이밍 기반 행동 가이드

### 5. 고민 상담
출생 정보와 카테고리를 기반으로 맞춤형 고민 상담 리포트를 생성합니다.

- **8개 카테고리**: 연애 / 취업 / 커리어 / 재정 / 사업 / 학업 / 인생 / 인간관계
- 3개월 트랜짓 프리뷰 + 피크 날짜 제공
- SSE 스트리밍으로 실시간 응답 전달

### 6. 반려동물 콘텐츠
- **별자리 성격 분석**: 생년월일 기반 10개 섹션 분석 (복슬신령님 캐릭터 보이스)
...
- 태양, 달, 수성, 금성, 화성, 목성, 토성, 키론의 황도 좌표 계산
- 출생 장소(위도/경도) 기반 상승궁(ASC) 계산
- 12하우스 배치, 행성 간 각도(Aspect), 역행(Retrograde) 분석
- 노드 축(North/South Node) 계산
- **트랜짓 계산**: 현재 천체 위치와 네이탈 차트 비교
- **비회원 프리뷰**: 네이탈 차트 요약을 미리보기로 제공하여 가입 전환 유도

### 8. SSE 실시간 스트리밍
리포트 생성 과정을 SSE(Server-Sent Events)로 실시간 전달하여, 사용자가 진행 상황을 확인할 수 있습니다.

```
Client ←──SSE──── Server
  │                  │
  │  [progress: 10%] │ ← 천체 계산 완료
  │  [progress: 45%] │ ← 리포트 생성 중
  │  [progress: 90%] │ ← 마무리
  │  [result: {...}]  │ ← 최종 리포트
  │  [complete]       │
```
...
│   ├── core/                   # Security, JWT, OAuth2, LLM, Toss, GA, Telegram
│   ├── domain/
